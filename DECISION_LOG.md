...
*   **Frontend Framework:** React with TanStack Start (v1.168.49) & TanStack Router.
    *   *Rationale:* TanStack Start integrates SSR (Server-Side Rendering) with type-safe server functions (`createServerFn`). This allows me to securely access environment API keys (Monday.com tokens and Gemini keys) on the server without leaking them to the client-side browser bundle.
*   **Bundler & Config:** Vite.
...
## 2. Key Assumptions
1.  **Column Identification Resilience:** Monday.com column IDs vary between accounts and imports. I assumed column headers can be identified by matching human-readable titles (e.g., `'Nature of Work'`, `'Masked Deal value'`, `'Execution Status'`) case-insensitively, and mapped them to standard camelCase properties in my SDK.
2.  **Deterministic Math:** LLMs are notorious for failing at simple arithmetic on large datasets. I assumed that all aggregations (sums, counts, averages, and group-bys) must be calculated deterministically on the server in TypeScript, and only passed to the LLM as verified context.
...
*   **Server-Side Pre-Aggregation vs. Raw Vector Embeddings:**
    *   *Trade-off:* I perform complete deterministic aggregation of the raw board rows on the server first, rather than feeding raw rows to the LLM or putting them in a vector database (RAG).
...
*   **Monday.com GraphQL REST fetch vs. SDK Package:**
    *   *Trade-off:* I implemented raw `fetch` calls to Monday's GraphQL endpoint in `BoardSDK.ts` instead of installing the official Monday Client SDK.
    *   *Why:* Keeps the project footprint small, avoids dependency version conflicts with Vite, and allows me to easily handle connection failure fallbacks.

---

## 4. Interpretation of "Leadership Updates"
I interpreted "Leadership Updates" as a request for a structured, executive-level synthesis of operational health across all business dimensions:
